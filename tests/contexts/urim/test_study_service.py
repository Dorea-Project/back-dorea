...
async def test_un_refus_ecrit_ecarte_l_option_sans_aiguiller():
    ...
    assert modele.aiguillages == []
    ecartees = [o[0] for o in apres.options if o[4]]
...
def test_le_rang_se_compte_dans_l_ordre_de_l_ecran_pas_dans_celui_du_moteur():
    ...
    dto = StudyDTO(
        ...
        options=(
            ("texte:c", "Celle qui résiste", "motif", "curation", False, "resiste", None),
            (
                "texte:a", "Celle qui en fait son sujet", "motif", "curation",
                False, "dominant", None,
            ),
        ),
    )
...
def test_une_option_ecartee_n_est_plus_une_cible_au_rang():
    ...
    dto = StudyDTO(
        ...
        rationale="Laquelle prêchez-vous ?",
        options=(
            ("axe:christologie", "Christologie", "motif", "locus", True, None, None),
            ("axe:ecclesiologie", "Ecclésiologie", "motif", "locus", False, None, None),
        ),
    )
